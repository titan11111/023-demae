...
class CubemapHandler extends ResourceHandler {
  ...
  loadAssets(cubemapAsset, callback) {
    ...
    const processTexAsset = function(index, texAsset2) {
      if (texAsset2.loaded && texAsset2.resource && !texAsset2.resource.device) {
        texAsset2.unload();
      }
      if (texAsset2.loaded) {
        onLoad(index, texAsset2);
      } else {
        registry.once(`load:${texAsset2.id}`, onLoad.bind(self, index));
        registry.once(`error:${texAsset2.id}`, onError.bind(self, index));
