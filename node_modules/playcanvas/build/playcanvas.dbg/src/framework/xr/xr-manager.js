...
const _XrManager = class _XrManager extends EventHandler {
  /**
  ...
  constructor(app) {
    ...
    if (this._supported && _XrManager._allowsSpatialTracking()) {
  ...
  static async isDeviceSupported(deviceType, type) {
    if (!platform.browser || !navigator.xr || !_XrManager._backendSupportsXr(deviceType)) {
      return false;
    }
    try {
  ...
  _sessionSupportCheck(type) {
    navigator.xr.isSessionSupported(type).then((available) => {
      if (available && !_XrManager._backendSupportsXr(this.app?.graphicsDevice?.deviceType)) {
        available = false;
      }
 ...
 * });
 */
__publicField(_XrManager, "EVENT_AVAILABLE", "available");
/**
 ...
 * });
 */
__publicField(_XrManager, "EVENT_START", "start");
/**
 ...
 * });
 */
__publicField(_XrManager, "EVENT_END", "end");
/**
 ...
 * });
 */
__publicField(_XrManager, "EVENT_UPDATE", "update");
/**
 ...
 * });
 */
__publicField(_XrManager, "EVENT_ERROR", "error");
let XrManager = _XrManager;
export {
  XrManager
};
