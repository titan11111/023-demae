...
class RotateGizmo extends TransformGizmo {
  ...
  constructor(camera, layer) {
     ...
     * @private
     */
    __publicField(this, "_selectionStartAngle", 0);
    /**
     ...
     * @private
     */
    __publicField(this, "_nodeLocalRotations", /* @__PURE__ */ new Map());
    /**
     ...
     * @private
     */
    __publicField(this, "_nodeRotations", /* @__PURE__ */ new Map());
    /**
     ...
     * @private
     */
    __publicField(this, "_guideAngleLines");
    /**
     * Internal copy of facing direction to avoid unnecessary updates.
     *
     * @private
     */
    __publicField(this, "_facingDir", new Vec3());
    /** @override */
    __publicField(this, "snapIncrement", 5);
    /**
     ...
     * @type {'absolute' | 'orbit'}
     */
    __publicField(this, "rotationMode", "absolute");
    this.setTheme({
      shapeBase: {
    ...
    this.on(TransformGizmo.EVENT_TRANSFORMSTART, (point2, x, y) => {
      this._screenPos.set(x, y);
      this._screenStartPos.set(x, y);
      this._selectionStartAngle = this._calculateArcAngle(point2, x, y);
      this._storeNodeRotations();
      this._storeGuidePoints();
      this._drag(true);
      this._angleGuide(true);
    });
    this.on(TransformGizmo.EVENT_TRANSFORMMOVE, (point2, x, y) => {
      ...
      if (axis === "xyz") {
        ...
        const delta = v2.copy(point2).sub(this._selectionStartPoint);
        ...
        let angleDelta = this._calculateArcAngle(point2, x, y) - this._selectionStartAngle;
        if (this.snap) {
  ...
  _angleGuide(state) {
    ...
    if (state && this.dragMode !== "show" && axis !== "xyz") {
      ...
      this._guideAngleLines[1].draw(
        gizmoPos,
        v1.copy(this._guideAngleEnd).add(gizmoPos),
        this._scale,
        baseColor
      );
  ...
  _calculateArcAngle(point2, x, y) {
    ...
    switch (this.rotationMode) {
      ...
      case "orbit": {
        ...
        const dir = v1.sub2(point2, this._camera.entity.getPosition()).normalize();
