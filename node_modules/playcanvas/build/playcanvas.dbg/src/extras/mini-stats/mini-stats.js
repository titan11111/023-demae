...
const _MiniStats = class _MiniStats {
  /**
   ...
   */
  constructor(app, options = _MiniStats.getDefaultOptions()) {
  ...
  static getDefaultOptions(extraStats = []) {
    ...
    if (extraStats.length > 0) {
      ...
      const extra = extraStats.flatMap((name) => _MiniStats.statPresets[name] ?? []).reverse();
 ...
 * @ignore
 */
__publicField(_MiniStats, "statPresets", {
  gsplats: [
